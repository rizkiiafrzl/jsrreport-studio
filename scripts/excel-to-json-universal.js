...
/**
 * Konversi file Excel ke JSON (Universal - auto detect provinsi/kabupaten)
 ...
 */
function excelToJsonUniversal(excelFilePath, outputPath = null, sheetName = null) {
    try {
        ...
        // Deteksi jenis file berdasarkan header
        const isKabupatenFile = headerRow.includes('NAMA KABUPATEN/KOTA') || 
                               headerRow.some(cell => String(cell).includes('KABUPATEN/KOTA'));
        
        console.log(`Jenis file: ${isKabupatenFile ? 'KABUPATEN/KOTA' : 'PROVINSI'}`);
        ...
        for (const row of dataRows) {
            ...
            // Mapping data sesuai dengan jenis file
            let dataItem;
            
            if (isKabupatenFile) {
                // Template kabupaten: No, Provinsi, Kabupaten_Kota, data...
                dataItem = {
                    No: rowNumber,
                    Provinsi: row[3] || '',
                ...
                // Template provinsi: No, Provinsi, data... (kabupaten sama dengan provinsi)
                dataItem = {
                    No: rowNumber,
                    Provinsi: row[3] || '',
                    Kabupaten_Kota: row[3] || '', // Untuk provinsi, kabupaten sama dengan provinsi
                    Jumlah: {
...
function convertAllExcelFiles(inputDir, outputDir) {
    try {
        ...
        for (const file of files) {
            ...
            console.log(`\nMengkonversi: ${file}`);
            try {
                excelToJsonUniversal(inputPath, outputPath);
...
if (require.main === module) {
    ...
    if (args.length === 0) {
        console.log('Usage:');
        console.log('  node excel-to-json-universal.js <excel-file> [output-file]');
        console.log('  node excel-to-json-universal.js --all <input-dir> <output-dir>');
        console.log('');
        console.log('Examples:');
        console.log('  node excel-to-json-universal.js "excel/KNODIK0003.xlsx" "data/KNODIK0003.json"');
        console.log('  node excel-to-json-universal.js --all "excel" "data"');
        process.exit(1);
    }
    ...
    if (args[0] === '--all') {
        ...
        excelToJsonUniversal(excelFile, outputFile);
    }
}
